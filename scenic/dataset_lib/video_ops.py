...
def _create_vectorized_fn(name, frames, prob, magnitude, replace_value,
                          cutout_const, translate_const):
  """Creates a function that applies `name` to all frames.

  Args:
    name: Name of the function.
    frames: The frames to apply the function to.
    prob: the probability of actually applying this instead of applying the
      identity function.
  ...
  Returns:
    ...
    frames (with probability prob) or the untouched frames (with probability
    1.0-prob).
  """
  ...
  def vectorized_fn():
    ...
    return tf.cond(
        should_apply_op,
        lambda: tf.map_fn((lambda f: func(f, *args)), frames),  # pylint: disable=unnecessary-lambda
        lambda: frames)
...
def distort_image_with_randaugment(frames,
  ...
  Args:
    frames: `Tensor` of shape [t, h, w, 3] with values in range [0-225] (will
      be clipped to this range if necessary) that represents a video.
  ...
  for _ in range(num_layers):
    ...
    for op_name, _ in available_ops:
      fn = _create_vectorized_fn(op_name, frames, magnitude, keep_prob,
                                 replace_value, cutout_const, translate_const)
