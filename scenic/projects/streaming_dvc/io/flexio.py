...
def _get_single_tfrecord_dataset(
  ...
  ds = tf.data.TFRecordDataset(tfrecords)  # pyrefly: ignore[bad-instantiation]
  ...
  if preprocess_fn is not None:
    if rng is not None:
      ds = flexio.apply_process_fn_with_populated_seed(
          ds, preprocess_fn, rng=rng)  # pyrefly: ignore[bad-argument-type]
    else:
  ...
  if postprocess_fn is not None:
    if rng is not None:
      ds = flexio.apply_process_fn_with_populated_seed(
          ds, postprocess_fn, rng=rng)  # pyrefly: ignore[bad-argument-type]
    else:
