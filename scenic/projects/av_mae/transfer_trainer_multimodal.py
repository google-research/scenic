...
def train_step(
  ...
  # vector.
  metrics['l2_grads'] = (jnp.sqrt(  # pyrefly: ignore[unsupported-operation]
      sum([jnp.vdot(g, g) for g in jax.tree_util.tree_leaves(grad)])), 1)
  metrics['l2_params'] = (jnp.sqrt(  # pyrefly: ignore[unsupported-operation]
      sum([jnp.vdot(p, p) for p in jax.tree_util.tree_leaves(new_params)])), 1)
  metrics['l2_updates'] = (jnp.sqrt(  # pyrefly: ignore[unsupported-operation]
...
def train(
  ...
  if (start_step == 0  # Which means "no" checkpoint is restored!
    ...
    if init_checkpoint_path is not None:
      ...
      train_state = model.init_from_train_state(train_state,  # pytype: disable=attribute-error
                                                restored_train_state,  # pyrefly: ignore[unbound-name]
                                                restored_model_cfg)
    ...
    for val_name, val_iter in valid_iter.items():  # pyrefly: ignore[missing-attribute]
      ...
      for _ in range(steps_per_eval):
        ...
        if not compute_map:
          ...
          eval_logits.append(logits_batch_in_cpu)  # pyrefly: ignore[unbound-name]
          eval_labels.append(labels_batch_in_cpu)  # pyrefly: ignore[unbound-name]
  ...
  for step in range(start_step + 1, total_steps + 1):  # pyrefly: ignore[unsupported-operation]
    ...
    if (config.dataset_configs.get('do_multicrop_test') and
        ((step % log_test_steps == 1) or step == total_steps)):  # pyrefly: ignore[unbound-name]
      ...
      with report_progress.timed('test'):
        ...
        for _ in range(steps_per_test):  # pyrefly: ignore[unbound-name]
          ...
          if not is_multilabel_model:
            ...
            # Return metrics, logits, labels.
            t_metrics, t_logits, t_labels = test_step_pmapped(  # pyrefly: ignore[unbound-name]
            ...
            all_test_logits.append(t_logits_in_cpu)  # pyrefly: ignore[unbound-name]
            all_test_labels.append(t_labels_in_cpu)  # pyrefly: ignore[unbound-name]
        ...
        if is_multilabel_model:
          ...
          all_test_logits_concat = np.concatenate(all_test_logits, axis=0)  # pyrefly: ignore[unbound-name]
          all_test_labels_concat = np.concatenate(all_test_labels, axis=0)  # pyrefly: ignore[unbound-name]
          additional_summary = evaluation_lib.compute_mean_avg_precision_dprime(
